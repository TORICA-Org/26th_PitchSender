...
// constexpr uint8_t SDA_PIN = 4; // 青
// constexpr uint8_t SCL_PIN = 5; // 黄
...
volatile EulerAngles lsm_angles;
...
void lsm_init() {
  ...
  if (lsm6_addr == 0) {
    mySerial.println("LSM6DSV16X not found.");
    while (1);
  }
...
void to_euler_angles() {
  ...
  lsm_angles.pitch = atan2(sinr_cosp, cosr_cosp) * 180.0f / PI;

  float sinp = 2.0f * (qua.w * qua.y - qua.z * qua.x);
  sinp = min(max(sinp, -1.0f), 1.0f);
  lsm_angles.roll = asin(sinp) * 180.0f / PI;
...
void lsm_refresh_euler() {
  ...
  // Serial.print("Euler: ");
  // Serial.printf("%.5f %.5f %.5f\n", lsm_angles.roll, lsm_angles.pitch, lsm_angles.yaw);
}